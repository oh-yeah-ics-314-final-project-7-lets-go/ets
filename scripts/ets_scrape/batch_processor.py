#!/usr/bin/env python3
"""
Batch processor for ATG CSEA IV&V Reports
...
class ATGBatchProcessor:
    def __init__(self):
        """Initialize batch processor"""
        self.parser = AIParser()
        self.atg_path = "downloads/ATG CSEA IV&/"
        
    def get_atg_pdfs(self) -> List[str]:
        """Get list of all ATG CSEA PDF files"""
        pdf_files = []
        atg_dir = Path(self.atg_path)
        
        if atg_dir.exists():
            for pdf_file in atg_dir.glob("*.pdf"):
...
"""
    ...
    def process_all_reports(self) -> Dict:
        """Process all ATG CSEA reports and return consolidated data"""
        pdf_files = self.get_atg_pdfs()
...
if __name__ == "__main__":
    ...
    # Process all ATG CSEA reports
    consolidated_data = processor.process_all_reports()
    
    if consolidated_data:
        # Save to output file
        output_path = "parsed_jsons/dhs_system_mod_project_complete_dataset.json"
